...
public class 리스트예제 {
	public static void main(String[] args) {
		...
		/*
		 * 리스트는 순서대로 0부터 +1해서 자리가 생기는 것이기 때문에 리스트.add(2,"파파야"); 0 = 1 1 = 비어있음 2 = 파파야
		 * 이런식으로 파파야를 추가하는 것은 자리값이 맞지 않는다는 에러가 발생하기 떄문에 번호 순서대로 값을 추가해줘야 함
		 */
		// list는 중복 추가 가능
		리스트.add(1, "파파야");
		System.out.println(리스트);
		...
		// 체리는 0번째 이기 때문에 1이 나오는 것이지 체리가 존재하지 않아서 0이 나오는 것이 아님
