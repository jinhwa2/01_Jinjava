...
키(key) - 값(value) 한 쌍의 집합을 이루는 구조
 ...
 키는 값에는 숫자를 넣어도 되고, 문자를 넣어도 됨, 문자열을 넣어도되고
		  어떤 값이 들어가도 괜찮음
  값 또한 모든 값이 들어갈 수 있음
  
  값은 중복이 가능하지만 키는 중복이 될 수 없음
  만약 중복된 키가 존재한다면 이전 키는 나중에 작성한 키로 대체됨
  순서가 보장되지 않음 만약 순서를 보장해야 하는 경우 LinkedHashMap을 사용할 수 있음
  ...
  메서드
   ...
   Map.Entry<자료형, 자료형>
     ...
     보통은 entrySet() 메서드를 사용해서 키-값 에 대한 정보를 보여줌
           모든 ~~~ 키와 값의 쌍을 한 번에 보여주지만
     ★Map.Entry 써서 모두 가져올 수 있음 //전체에서 하나씩 하나씩 꺼내서 보여줌
      맵 안에서 각각의 키-값
