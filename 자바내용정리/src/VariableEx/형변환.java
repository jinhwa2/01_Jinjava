...
/*
 (자료)형변환 
	...
	큰 byte에서 작은byte로 넘어갈 때는 크 그릇에서 작은 그릇으로
	이동하기때문에 강제로 넣어줘야함 (강제로 넣을 경우 데이터 손실 발생 가능성 있음)
	...
	★메서드 호출을 통한 형변환
	  →  숫자자료형
		...
		Byte.pareseByte(변환할 String 변수);
		Short.pareseShort(변환할 String 변수);
		Float.pareseFloat(변환할 String 변수);
		Long.pareseLong(변환할 String 변수)ㅣ
